...
- **Diagram**: See :numref:`fig_softmaxreg`.
- **Structure**:
...
- **Gibbs (1902)**: Adapted softmax ideas from physics :cite:`Gibbs.1902`.
- **Boltzmann**: Modeled distribution over energy states in gas molecules:
  - Prevalence of a state proportional to $\exp(-E/kT)$.
  - $E$ = energy of a state, $T$ = temperature, $k$ = Boltzmann constant.
  - Changing "temperature" $T$ adjusts preference for energy states.
  
- **Energy-Based Models**:
  - Energy equates to error in statistical systems.
  - Used in deep learning to describe problems :cite:`Ranzato.Boureau.Chopra.ea.2007`.
...
- Method: Maximum likelihood estimation.
  - Same concept as used for probabilistic justification of mean squared error loss in :numref:`subsec_normal_distribution_and_squared_loss`.
...
- **Definition**: Loss function in :eqref:`eq_l_cross_entropy` is called *cross-entropy loss*.
...
- **Combining Equations**:
  - Plugging :eqref:`eq_softmax_y_and_o` into the definition of the loss in :eqref:`eq_l_cross_entropy`:
...
- **Mathematical Consistency**:
  - The same loss function $l$ in :eqref:`eq_l_cross_entropy` applies.
...
- **Fundamental Theorem**: To encode data drawn randomly from distribution $P$, we need at least $H[P]$ "nats" to encode it :cite:`Shannon.1948`.
...
- **Expected Surprisal**: Entropy defined in :eqref:`eq_softmax_reg_entropy` is the *expected surprisal* when probabilities match the data-generating process.
...
- **Parameterization and Cost**: For fully connected layers with $d$ inputs and $q$ outputs, the cost is $\mathcal{O}(dq)$.
  - High cost in practice, but can be reduced through approximation and compression.

### Reducing Computational Cost

- **Deep Fried Convnets**: Reduce cost using permutations, Fourier transforms, and scaling :cite:`Yang.Moczulski.Denil.ea.2015`.
- **Structural Matrix Approximations**: Techniques for reducing computational cost :cite:`sindhwani2015structured`.
- **Quaternion-like Decompositions**: Reduce cost to $\mathcal{O}(\frac{dq}{n})$ with a trade-off in accuracy for computational and storage efficiency :cite:`Zhang.Tay.Zhang.ea.2021`.
...
4. Use weights $\beta_i$ for training on $\{(\mathbf{x}_1, y_1), \ldots, (\mathbf{x}_n, y_n)\}$ in :eqref:`eq_weighted-empirical-risk-min`.
...
- **Calculating Weights**:
  ...
  - Use weights in weighted empirical risk minimization in :eqref:`eq_weighted-empirical-risk-min`.
